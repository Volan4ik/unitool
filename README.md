...
1. Configure `.env`:
   ...
   - `LOG_FILE_PATH` (default: `./logs/bot.log`; leave empty to disable file logging; Docker image default: `/data/logs/bot.log` for Amvera persistent storage)
   ...
   - `METRICS_TOKEN` (optional; enables protected `GET /metrics` on `HTTP_ADDR` when set)
   - `TELEGRAM_TOKEN`
   - `PROVIDER_TOKEN`
...
- `GET /healthz`
- `GET /readyz`
- `GET /metrics` with `Authorization: Bearer $METRICS_TOKEN` when `METRICS_TOKEN` is set
