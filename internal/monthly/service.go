...
func (s *Service) applyMonthlyFree(ctx context.Context) error {
    ...
    // Single INSERT for all users who don't have monthly_free for current month
    const sql = `
INSERT INTO credit_ledger (user_id, delta_text, delta_image, delta_video, delta_search, reason, meta)
SELECT u.id, $1::int, $2::int, $3::int, $4::int, 'monthly_free', $5::jsonb
FROM users u
WHERE NOT EXISTS (
  SELECT 1 FROM credit_ledger c
  WHERE c.user_id = u.id
    AND c.reason = 'monthly_free'
